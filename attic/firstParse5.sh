...
line="$4"
		...
		if [ -f "${thepwd}/buildlog/$(basename ${line%_decompiled.fileList}_vanilla.built)" ]; then
			if [ -f "${thepwd}/buildlog/$(basename ${line%_decompiled.fileList}_vanilla.failed)" ]; then
				rm "${thepwd}/buildlog/$(basename ${line%_decompiled.fileList}_vanilla.failed)"
			fi
			found="0"
		fi
