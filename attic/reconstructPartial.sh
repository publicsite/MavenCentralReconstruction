	...
	if [ -d ${groupId}/${artifactId}/${version} ]; then
		...
		if [ -d "${groupId}/${artifactId}/${version}/extractedSources" ]; then
			...
			find "${groupId}/${artifactId}/${version}/extractedSources" -name "*.java" | while read line; do
				...
				className="$(printf "%s\n" "$lineNoComments" | grep -o "^.*\s*class .*" | head -n 1 | grep -o "class.*" | cut -d " " -f 2 | cut -d "<" -f 1 | cut -d "{" -f 1 | cut -d "(" -f 1 | tr -d '\n' | sed "s#\r##g" )"

				if [ "${packageName}" != "" ]; then

				if [ "${className}" = "" ]; then
					className="$(printf "%s\n" "$lineNoComments" | grep -o "^.*\s*interface .*" | head -n 1 | grep -o "interface.*" | cut -d " " -f 2 | cut -d "<" -f 1 | cut -d "{" -f 1 | cut -d "(" -f 1 | tr -d '\n' | sed "s#\r##g")"
				fi
