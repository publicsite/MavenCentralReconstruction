...
if [ -f "$line/build.gradle" ]; then
echo "${line}/build.gradle" | cut -c 3-
else
...
if [ ! -f "pom.xml" ]; then
	...
	first=0
	repositories="$(cat build.gradle | tr '\n' '^' | grep -o "\^repositories.*{.*}" | cut -d "}" -f1)"
...
#print dependencies
	...
	internalps="$(cat build.gradle | grep -o "[[:alnum:]].*$" \
